...
|---|---|---|
| `versionCode` erhöhen | Steht aktuell auf 36 / „2.27" (`app/build.gradle.kts`) und 100010 / „0.2.8-wear" (`wear/build.gradle.kts`). Vor jedem Upload weiterzählen — Play lehnt einen bereits benutzten Code ab. | beim Release |
...
| Beinamen im Paritäts-Vertrag | **Der Grund ist entfallen, der Nutzen bleibt klein.** Der Rahmen steht in `parity/golden-vectors.txt`; der Beiname stand nicht drin, weil die Karte Android-allein war. Seit v2.26 zeichnet EINE geteilte Routine in `:ui` die Karte für beide Plattformen — einen zweiten Port, der driften könnte, gibt es nicht mehr. Was bliebe, wäre reine Golden-Master-Abdeckung der Beinamen-Schwellen (die `CardStyleTest` bereits prüft). Aufnehmen lohnt erst, wenn die Schwellen je Plattform unterschiedlich interpretiert werden könnten — heute nicht der Fall. |
